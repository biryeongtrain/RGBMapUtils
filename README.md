...
- `/rgbmapdebug`: `src/main/resources/img.png`를 원본 해상도 기준(64x64 단위 타일)으로 분할 인코딩해 월드에 표시합니다.
  - 인벤토리 지급 없이 `VirtualDisplay`로 플레이어가 보는 위치의 블록 면에 맵 패널 2개를 나란히 띄웁니다.
  - 왼쪽: 원본 `BufferedImage`를 `MapCanvas`로만 변환한 멀티맵(팔레트 근사)
  - 오른쪽: `RgbMapCodec` 인코딩 멀티맵
  - 이미지가 `64`의 배수가 아니면 마지막 타일의 남는 영역은 검정색으로 패딩됩니다.
